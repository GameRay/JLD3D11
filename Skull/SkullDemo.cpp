...
public:SkullApp(HINSTANCE hInstance);
	   ~SkullApp();
...
void SkullApp::BuildGeometryBuffers()
{
	...
	{
		fin>>indices[i*3+0]>>indices[i*3+1]>>indices[3*i+2];
	}
	fin.close();
...
//
//	float nx, ny, nz;
//	XMFLOAT4 black(1.0f, 1.0f, 1.0f, 1.0f);
//
//	std::vector<Vertex> vertices(vcount);
//	for(UINT i = 0; i < vcount; ++i)
//	{
...
bool SkullApp::Init()
{
	...
	D3D11_RASTERIZER_DESC wirefraneDesc;
	ZeroMemory(&wirefraneDesc,sizeof(D3D11_RASTERIZER_DESC));
	wirefraneDesc.FillMode=D3D11_FILL_WIREFRAME;
	wirefraneDesc.CullMode=D3D11_CULL_BACK;
	wirefraneDesc.FrontCounterClockwise=false;
	wirefraneDesc.DepthClipEnable=true;

	HR(md3dDevice->CreateRasterizerState(&wirefraneDesc,&mWireframeRS));
	return true;
}
